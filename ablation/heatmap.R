...
dds.bulk <- DESeq(DESeqDataSetFromMatrix(counts.filtered[,conditions$procedure == "bulk"], data.frame(tissue = conditions$tissue[conditions$procedure == "bulk"]), ~ tissue), parallel = T)
...
ggsave("heatmap_hundreds.pdf", heatmap.hundreds, width = 6, height = 4.5)
