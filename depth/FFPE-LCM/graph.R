...
result <- do.call(rbind, lapply(c("single", "bulk"), function(amount) cbind(
	rbind(
...
result$amount <- factor(as.character(result$amount))
...
ggsave("depth_ffpe-lcm.pdf", combined.plot, "pdf", width = 12, height = 4)
